"""
Training script with Mask-Guided Part-Level Contrastive Loss.
Modified from Retrieval.py to use ALBEF_Masked model and masked dataset.

Usage:
    # Single GPU
    python Retrieval_masked.py --config configs/PS_icfg_pedes_masked.yaml \
        --output_dir output/icfg-pedes-masked --distributed False

    # Multi-GPU
    python -m torch.distributed.launch --nproc_per_node=1 Retrieval_masked.py \
        --config configs/PS_icfg_pedes_masked.yaml --output_dir output/icfg-pedes-masked
"""
...
import utils
from dataset.ps_dataset_masked import ps_train_dataset_masked
from dataset.ps_dataset import ps_eval_dataset
from dataset import create_sampler, create_loader
from models.model_person_search_masked import ALBEF_Masked
...
def create_masked_dataset(config):
    """Create datasets with mask support for training."""
    ...
    train_dataset = ps_train_dataset_masked(
        config['train_file'], train_transform, config['train_image_root'],
        mask_root=config['mask_root'],
        max_words=config['max_words'],
        weak_pos_pair_probability=config['weak_pos_pair_probability'],
        image_res=config['image_res'],
    )
    val_dataset = ps_eval_dataset(
        config['val_file'], test_transform, config['val_image_root'], config['max_words']
    )
    test_dataset = ps_eval_dataset(
        config['test_file'], test_transform, config['test_image_root'], config['max_words']
    )
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    metric_logger.add_meter('loss_part', utils.SmoothedValue(window_size=1, fmt='{value:.4f}'))
    ...
    for i, (image1, image2, text1, text2, idx, replace, masks) in enumerate(
        ...
        text_input1 = tokenizer(text1, padding='longest', max_length=config['max_words'],
                                return_tensors="pt").to(device)
        text_input2 = tokenizer(text2, padding='longest', max_length=config['max_words'],
                                return_tensors="pt").to(device)
        ...
        loss_cl, loss_pitm, loss_mlm, loss_prd, loss_mrtd, loss_part = model(
            image1, image2, text_input1, text_input2,
            alpha=alpha, idx=idx, replace=replace, masks=masks
        )

        # Weighted sum of all losses
        losses = (loss_cl, loss_pitm, loss_mlm, loss_prd, loss_mrtd, loss_part)
        loss = sum(w * l for w, l in zip(config['weights'], losses))
...
def evaluation(model, data_loader, tokenizer, device, config):
    """Same evaluation as original — masks not needed for inference."""
    model.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Evaluation:'
    ...
    print('Evaluation time {}'.format(str(datetime.timedelta(seconds=int(total_time)))))
...
def itm_eval(scores_t2i, img2person, txt2person, eval_mAP):
    ...
    eval_result = {'r1': ir1, 'r5': ir5, 'r10': ir10, 'r_mean': ir_mean}
    if eval_mAP:
        ...
        eval_result['mAP'] = AP.mean().item() * 100.0
...
def main(args, config):
    ...
    # Dataset (with masks)
    print("Creating masked retrieval dataset")
    train_dataset, val_dataset, test_dataset = create_masked_dataset(config)
    ...
    # Model (masked version)
    print("Creating ALBEF_Masked model")
    model = ALBEF_Masked(config=config, text_encoder=args.text_encoder, tokenizer=tokenizer)
    ...
    # Initialize memory bank by forwarding all training samples
    if hasattr(model_without_ddp, 'memory_bank'):
        # Create a sequential loader for initialization (no shuffle, no drop_last)
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if epoch >= config['eval_epoch'] or args.evaluate:
            ...
            if utils.is_main_process():
                test_result = itm_eval(score_test_t2i, test_dataset.img2person,
                                       test_dataset.txt2person, args.eval_mAP)
                print('Test:', test_result, '\n')

                log_stats = {'epoch': epoch}
                if not args.evaluate:
                    log_stats.update({f'train_{k}': v for k, v in train_stats.items()})
                log_stats.update({f'test_{k}': v for k, v in test_result.items()})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='./configs/PS_icfg_pedes_masked.yaml')
    parser.add_argument('--output_dir', default='output/icfg-pedes-masked')
