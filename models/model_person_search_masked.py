"""
ALBEF model extended with Mask-Guided Part Pooling + Part-Level Contrastive Loss.
This is a drop-in replacement for model_person_search.py.

Changes from original:
1. Added MaskGuidedPartPooling module
2. Added PartContrastiveLoss module
3. forward() accepts `masks` parameter and returns additional loss_part
"""
...
class ALBEF_Masked(nn.Module):
    def __init__(self, text_encoder=None, tokenizer=None, config=None):
        ...
        # === NEW: Mask-guided modules ===
        part_proj_dim = config.get('part_proj_dim', 256)
        self.part_pooling = MaskGuidedPartPooling(
            embed_dim=vision_width, proj_dim=part_proj_dim
        )
    ...
    @property
    def patch_grid(self):
        """Get the spatial dimensions of the patch grid."""
        p = self.patch_size
        return (self.image_res // p, self.image_res // p)

    def forward(self, image1, image2, text1, text2, alpha, idx, replace, masks=None):
        """
        Same as original ALBEF forward, with additional `masks` parameter.
        masks: [B, image_res, image_res] — integer tensor with part IDs 0-6.
        Returns: loss_cl, loss_pitm, loss_mlm, loss_prd, loss_mrtd, loss_part
        """
        # ===== Image encoding =====
        ...
        # ===== Contrastive loss (same as original) =====
        idx = idx.view(-1, 1)
        ...
        with torch.no_grad():
            ...
            mask_diag = torch.eq(idx, idx.T)
            weights_i2t.masked_fill_(mask_diag, 0)
            weights_t2i.masked_fill_(mask_diag, 0)
